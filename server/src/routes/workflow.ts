  ...
  try {
    ...
    if (result.rows.length === 0) {
      const created = await pool.query(
        "INSERT INTO workflow_config (name, workflow_type, created_by, updated_by) VALUES ('default', 1, 'system', 'system') RETURNING *"
      )
...
router.put('/config', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE workflow_config SET workflow_type = $1, updated_at = NOW(), updated_by = 'system' WHERE name = 'default' RETURNING *`,
      [workflow_type]
    )
    if (result.rows.length === 0) {
      const created = await pool.query(
...
router.post('/ingest', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO finding_reviews (asset_id, finding_qid, finding_title, workflow_type, status, assigned_to, published_at, created_by, updated_by)
...
router.post('/reviews/:id/l1-decision', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE finding_reviews SET status = $1, l1_reviewer = $2, l1_decision = $3, l1_decided_at = NOW(), assigned_to = $4, published_at = $5, updated_by = 'system', updated_at = NOW() WHERE id = $6 RETURNING *`,
...
router.post('/reviews/:id/l2-decision', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE finding_reviews SET status = $1, l2_approver = $2, l2_decision = $3, l2_decided_at = NOW(), assigned_to = $4, published_at = $5, updated_by = 'system', updated_at = NOW() WHERE id = $6 RETURNING *`,
